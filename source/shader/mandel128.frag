...
uniform vec4 uPositionRe;
uniform vec4 uPositionIm;
uniform vec4 uFactorRe;
uniform vec4 uFactorIm;
...
vec4 qs(vec2 dsa)
{
	return vec4(dsa.x,dsa.y,0.0,0.0);
}

vec4 qs_mandel_sample(vec4 c_re, vec4 c_im)
{
	int i;
	vec4 z_re = vec4(0.0,0.0,0.0,0.0);
	vec4 z_im = vec4(0.0,0.0,0.0,0.0);
	for(i = 0; i < uDepth && qs_compare(qs_add(qs_mul(z_re,z_re),qs_mul(z_im,z_im)),qs(ds(16.0))) < 0.0; ++i) 
	{
	    vec4 t = qs_add(qs_add(qs_mul(z_re,z_re),-qs_mul(z_im,z_im)),c_re);
	    z_im = qs_add(qs_mul(qs(ds(2.0)),qs_mul(z_re,z_im)),c_im);
	    z_re = t;
	}
	...
	{
		float smooth_i = float(i) + 1.0 - log(log(length(vec2(z_re.x,z_im.x))))/log(2.0);
...
void main(void) 
{
	...
	for(int ix = 0; ix < uSamples; ++ix)
	{
		...
		{
			vec2 d = vDeviation + vec2(0.5*(step-1.0) + step*float(ix),0.5*(step-1.0) + step*float(iy));
			vec4 c_re = qs_add(uPositionRe,qs_add(qs_mul(uFactorRe,qs(ds(d.x))),-qs_mul(uFactorIm,qs(ds(d.y)))));
			vec4 c_im = qs_add(uPositionIm,qs_add(qs_mul(uFactorRe,qs(ds(d.y))), qs_mul(uFactorIm,qs(ds(d.x)))));
			color += qs_mandel_sample(c_re,c_im);
		}
	}
